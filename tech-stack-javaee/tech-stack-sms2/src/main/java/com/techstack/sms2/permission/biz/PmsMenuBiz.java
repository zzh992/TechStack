 ...
 */
@Service("pmsMenuBiz")
public class PmsMenuBiz extends BaseBiz{
	...
	private void recursionTreeMenu(Long pId, StringBuffer buffer, List list, String url) {
		...
		List<PmsMenu> listMenu = getSonMenuListByPid(pId, list);
		for (PmsMenu menu : listMenu) {
	 ...
	 */
	@SuppressWarnings({ "rawtypes", "unchecked" })
	private List<PmsMenu> getSonMenuListByPid(Long pId, List<PmsMenu> menuList) {
		List sonMenuList = new ArrayList<PmsMenu>();
	...
	private void buildAdminPermissionTree(Long pId, StringBuffer treeBuf, List menuList) {
		...
		List<PmsMenu> sonMenuList = getSonMenuListByPid(pId, menuList);
		for (PmsMenu sonMenu : sonMenuList) {
			//String id = map.get("id").toString();// id
			//String name = map.get("name").toString();// 名称
			...
			String navTabId = "";
			if (StringUtil.isNotNull(sonMenu.getTargetName())) {
	 ...
	 * @return void
	 */
	public void createMenu(PmsMenu model){
		try {
	...
	private void buildPermissionTree(Long pId, StringBuffer treeBuf, List allMenuList, String menuIds, String actionIds) {
		...
		List<PmsMenu> sonMenuList = getSonMenuListByPid(pId, allMenuList);
		for (PmsMenu sonMenu : sonMenuList) {
	...
	private void recursionTreeMenuLookUp(Long pId, StringBuffer buffer, List list) {
		...
		List<PmsMenu> sonMenuList = getSonMenuListByPid(pId, list);
		for (PmsMenu sonMenu : sonMenuList) {
			//String id = map.get("id").toString();// id
			//String parentId = map.get("pId").toString();// 父id
			//String name = map.get("name").toString();// 名称
